...
use http_service::http_service::file_service::service::biz_service::{
    create_avatar_biz, get_pub_file_record_by_biz_id,
};
use http_service::http_service::file_service::service::file_service::{
    get_file_by_path, get_file_record_by_id, upload_file_local,
};
...
pub async fn download_pub_biz(rb: &RBatis, biz_id: String) -> Result<HttpResponse, anyhow::Error> {
    ...
    if file_id_vec.len() > 1 {
        let res = CommonResponseRef::<Vec<&str>>::success_json(&file_id_vec)?;
