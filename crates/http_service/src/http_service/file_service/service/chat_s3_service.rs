...
/// 上传聊天文件到S3（预览文件）
/// 
/// 将压缩后的预览文件上传到 chat-file-preview 桶
/// 
/// # 参数
/// - `rb`: 数据库连接
/// - `user_id`: 用户ID
...
/// # 返回值
/// 返回文件上传记录
pub async fn upload_chat_preview_file_s3(
    rb: &RBatis,
    user_id: String,
    payload: Multipart,
    s3_client: Arc<S3Client>,
) -> Result<FileUploadRecord, anyhow::Error> {
    let bucket = s3_client.config.chat_file_preview_bucket.clone();
    upload_chat_file_to_bucket(rb, user_id, payload, s3_client.clone(), bucket).await
}

/// 上传聊天文件到S3（原文件）
/// 
/// 将原始文件上传到 chat-file-origin 桶
/// 
/// # 参数
/// - `rb`: 数据库连接
/// - `user_id`: 用户ID
...
/// # 返回值
/// 返回文件上传记录
pub async fn upload_chat_origin_file_s3(
    rb: &RBatis,
    user_id: String,
    payload: Multipart,
    s3_client: Arc<S3Client>,
) -> Result<FileUploadRecord, anyhow::Error> {
    let bucket = s3_client.config.chat_file_origin_bucket.clone();
    upload_chat_file_to_bucket(rb, user_id, payload, s3_client.clone(), bucket).await
}

/// 上传聊天文件到指定桶
async fn upload_chat_file_to_bucket(
    rb: &RBatis,
    user_id: String,
    ...
    if let Some(filename) = content_disposition.get_filename() {
        ...
        if !file_upload_record_exist.is_empty() {
            warn!("file already exists (S3 chat): {}", filename);
            ...
            info!("S3 chat file uploaded successfully: key={}, size={}", storage_info.key, storage_info.size);
...
/// 从S3下载聊天文件
pub async fn download_chat_file_s3(
    s3_client: Arc<S3Client>,
    file_record: &FileUploadRecord,
) -> Result<Vec<u8>, anyhow::Error> {
    let s3_key = file_record.file_path.as_ref().ok_or(anyhow!("S3对象key为空"))?;
    
    // 聊天文件可能在 preview 或 origin 桶中，根据文件路径判断或使用默认origin桶
    // 简单处理：统一使用 origin 桶（因为新上传的文件key不再包含bucket前缀）
    let bucket = s3_client.config.chat_file_origin_bucket.clone();
    
    let storage = S3Storage::with_bucket(s3_client.clone(), bucket);
    let data = storage
        .download(s3_key)
        .await
...
/// 从S3生成预签名下载URL
pub async fn get_chat_s3_presigned_download_url(
    s3_client: Arc<S3Client>,
    file_record: &FileUploadRecord,
    ...
    // 聊天文件可能在 preview 或 origin 桶中，统一使用 origin 桶
    let bucket = s3_client.config.chat_file_origin_bucket.clone();
...
/// 删除S3上的聊天文件
pub async fn delete_chat_file_s3(
    s3_client: Arc<S3Client>,
    file_record: &FileUploadRecord,
) -> Result<(), anyhow::Error> {
