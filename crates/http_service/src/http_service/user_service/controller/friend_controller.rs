...
pub async fn add_friend_api(
    ...
    let res = add_friend(state.db(), friend).await;
    if let Err(t) = res {
        error!("err_context {:?}", t);
        error!("{}", t.backtrace());
...
pub async fn process_friend_api(
    ...
    let res = process_friend(state.db(), friend).await;
    if let Err(t) = res {
        error!("err_context {:?}", t);
        error!("{}", t.backtrace());
        return HttpResponse::BadRequest()
...
pub async fn get_friend_api(
    ...
    respond_json_any!(get_friend_list(state.db(), uuid, last_uuid, version).await)
}
...
pub async fn get_accept_friend_request_list_api(
    ...
    respond_json_any!(get_accept_friend_request_list(state.db(), uuid, accept_status).await)
}
...
pub async fn get_friend_request_list_api(
    ...
    respond_json_any!(get_friend_request_list(state.db(), uuid, accept_status).await)
}
...
pub async fn delete_friend_api(
    ...
    let res = delete_friend_service(state.db(), my_uuid, friend_uuid).await;
    if let Err(t) = res {
        error!("err_context {:?}", t);
        error!("{}", t.backtrace());
        return HttpResponse::BadRequest()
