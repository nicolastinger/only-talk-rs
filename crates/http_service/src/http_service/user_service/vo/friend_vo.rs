...
pub async fn query_friend_list(
    ...
    let friend_list: Option<Vec<FriendListVO>> = rb
        .exec_decode("select bu.uuid, bu.username, bu.account, bu.icon, bu.info, bu.user_type, fs.is_del, fs.is_block, fs.version, fs.updated_at, fs.created_at from
...
pub async fn query_black_list(rb: &RBatis, uuid: &Uuid) -> Result<String, anyhow::Error> {
    let black_list: Option<Vec<BlackListVO>> = rb
        .exec_decode("select bu.uuid, bu.account, bu.username, bu.icon, bu.user_type, bl.created_at from black_list bl left join basic_user bu on bl.block_user = bu.uuid where bl.me_user = ?", vec![value!(uuid)])
        .await?;
