...
async fn create_test_client() -> S3Client {
    ...
    S3Client::new(config).await.expect("创建 S3 客户端失败")
}
...
async fn test_ensure_default_bucket() {
    ...
    // 这个操作应该是幂等的
    client.ensure_default_bucket().await.expect("确保默认桶失败");
}
...
async fn test_upload_and_download() {
    ...
    // 上传
    let upload_info =
        storage.upload(test_key, test_data.clone(), Some("text/plain")).await.expect("上传失败");
...
async fn test_download_range() {
    ...
    // 范围下载: 读取前10个字节
    let range_data = storage.download_range(test_key, 0, 9).await.expect("范围下载失败");

    assert_eq!(range_data, b"0123456789");

    // 范围下载: 读取中间部分
    let range_data = storage.download_range(test_key, 10, 19).await.expect("范围下载失败");
...
async fn test_list_objects() {
    ...
    // 上传几个测试文件
    let test_files = vec!["test/list/file1.txt", "test/list/file2.txt", "test/list/file3.txt"];

    for key in &test_files {
        storage.upload(key, b"test content".to_vec(), Some("text/plain")).await.expect("上传失败");
    }

    // 列举对象
    let objects = storage.list(Some("test/list/"), Some(100)).await.expect("列举失败");
...
async fn test_copy_object() {
    ...
    // 上传源文件
    storage.upload(src_key, test_data.clone(), Some("text/plain")).await.expect("上传失败");
...
async fn test_batch_delete() {
    ...
    // 上传多个文件
    let test_files = vec!["test/batch/del1.txt", "test/batch/del2.txt", "test/batch/del3.txt"];
    ...
    // 批量删除
    let failed = storage.delete_batch(&test_files).await.expect("批量删除失败");
...
async fn test_large_file_upload() {
    ...
    let test_data: Vec<u8> = (0..15 * 1024 * 1024).map(|i| (i % 256) as u8).collect();
