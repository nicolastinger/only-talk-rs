...
CREATE TABLE IF NOT EXISTS biz_record (
    ...
    biz_type VARCHAR(100),                       -- 业务类型(头像、用户背景、广场图片等)
    remark TEXT                                  -- 备注信息
);
...
COMMENT ON COLUMN biz_record.preview_file_ids IS '关联的压缩后的文件UUID';
...
COMMENT ON COLUMN biz_record.biz_type IS '业务类型(头像、用户背景、广场图片等)';
