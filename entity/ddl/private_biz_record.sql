...
CREATE TABLE IF NOT EXISTS private_biz_record (
    ...
    biz_type varchar(100), -- 业务类型(朋友圈、收藏、私密相册等)
    remark text -- 备注信息
);
...
COMMENT ON COLUMN public.private_biz_record.biz_name IS '业务名称';
COMMENT ON COLUMN public.private_biz_record.description IS '业务描述';
COMMENT ON COLUMN public.private_biz_record.file_ids IS '关联的文件UUID';
COMMENT ON COLUMN public.private_biz_record.preview_file_ids IS '关联的压缩后的文件UUID';
COMMENT ON COLUMN public.private_biz_record.created_by IS '创建用户UUID';
COMMENT ON COLUMN public.private_biz_record.created_at IS '创建时间（Unix时间戳，毫秒）';
COMMENT ON COLUMN public.private_biz_record.updated_at IS '更新时间（Unix时间戳，毫秒）';
COMMENT ON COLUMN public.private_biz_record.status IS '业务状态（0-正常，1-已禁用，2-已删除）';
COMMENT ON COLUMN public.private_biz_record.approve_status IS '审批状态(0-待处理,1-已通过,2-已拒绝)';
