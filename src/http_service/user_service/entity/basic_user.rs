...
pub struct BasicUser {
    ...
    #[validate(
        required(message = "需要输入名称"),
        length(min = 5, message = "账号长度必须大于5")
    )]
    pub username: Option<String>,
    #[validate(
        required(message = "需要输入id"),
        length(min = 5, message = "账号长度必须大于5")
    )]
    ...
    #[validate(
        ...
        regex(
            path = "crate::utils::validators::PASSWORD_REGEX",
...
pub async fn get_raw_sql(rb: web::Data<RBatis>) {
    let table: Option<Vec<HashMap<String, serde_json::Value>>> = rb
        .query_decode(
            "select * from rust_user_test where id = ? limit ?",
            vec![rbs::to_value!("huangjinsheng"), rbs::to_value!(1)],
        )
        .await
        .unwrap();
    if let Some(t) = table {
