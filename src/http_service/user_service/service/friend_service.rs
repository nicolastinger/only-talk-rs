use std::str::FromStr;
use anyhow::anyhow;
use crate::http_service::user_service::entity::friend::{FriendLink};
...
use crate::quic_service::msg_service::process_msg_service::send_system_msg;
use crate::http_service::user_service::dto::friend_request_info_dto::FriendRequestInfoDTO;
use crate::http_service::user_service::entity::friend_request_info::FriendRequestInfo;
...
pub async fn add_friend(
    ...
    let request_user = friend_request_info_dto.request_user.ok_or_else(|| anyhow!("request_user is None"))?;
    let request_user = rbatis::rbdc::Uuid::from_str(request_user.as_str())?;
    let accept_user_str = friend_request_info_dto.accept_user.ok_or(anyhow!("accept_user is None"))?;
    ...
    // 查询之前添加的申请
    let friend_request_info = FriendRequestInfo::select_by_uuid(rb, &request_user, &accept_user).await?;
    ...
    // 向接受端发送消息
    send_system_msg(accept_user_str, MessageType::System, USER_ADD_FRIEND.to_string()).await?;
    result
}
...
pub async fn process_friend(
    ...
    // 向接受端发送消息
    send_system_msg(request_user_str, MessageType::System, USER_PROCESS_FRIEND.to_string()).await?;
    result
}
