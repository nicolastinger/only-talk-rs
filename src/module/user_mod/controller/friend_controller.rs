...
use crate::module::user_mod::service::friend::{fn_add_friend, get_friend_by_id};
use crate::utils::dto::{AuthAccount, ReqList};
use crate::utils::http_response::{CommonResponse, CommonResponseRef};
...
#[post("/add_friend")]
pub async fn add_friend(req: HttpRequest, state: web::Data<RBatis>, friend: web::Json<FriendLinkDTO>) -> impl Responder {
    let friend = validate_and_respond!(friend);
    respond_json_any!(fn_add_friend(state.as_ref()).await)
}
